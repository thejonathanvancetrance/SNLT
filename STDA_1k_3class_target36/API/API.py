...
def getBestModel(N, xDf, yDf, emptyModel, paramGrid, features, metricToOptimize='r2'):
    ...
    ## return the results
    return avgMAE, avgRSq, bestMAE, bestRSq, bestModel, featuresUsed, accuracy, f1, mcc, truth, predictions
...
def saveMLResults(xTest, yTest, N, xDf, yDf, modelList, workingDir, numFeatures, printResults=True):
    ...
    for tup in modelList:
        ...
        # get results
        avgMAE, avgRSq, bestMAE, bestRSq, bestModel, bestFeatures, accuracy, f1, mcc, truth, predictions = getBestModel(N, xDf, yDf, model, paramGrid, 
                                                                           features=numFeatures, metricToOptimize='r2')

        #print("truth: ", truth)
        #print("predictions: ", predictions)
        real_accuracy = accuracy_score(truth, predictions)
        ...
        print("MAE: ", m)
        print("R sq: ", r)
        print("accuracy score for DA/TDA: ", accuracy)
            
        #count += 1

def actualTest(xTest,yTest,model):
...
#     mae = str(meanAbsoluteError)
    ...
    accuracy = accuracy_score(yTest, pred)
    print('TDA yTest: ', yTest)
    print('TDA preds: ', pred)
    
    return(meanAbsoluteError, rSq, accuracy)
